...
try {
    ...
    // 3. Pula as 7 linhas de cabeçalho (com base no seu script Python)
    for ($i = 0; $i < 7; $i++) {
        fgetcsv($handle, 1000, ";");
    }
    ...
    // 5. Lê linha por linha
    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
        if (empty($data[0]) || empty($data[1])) {
            continue; // Pula linhas sem código ou item
        }

        $codigo_sap = ltrim(trim($data[0]), '0');
        $item = trim($data[1]);
        $grupo = trim($data[2]);
        ...
        $stmt_insert->execute([
            ...
            $pedido_vd,
            $total_caixa,
            $total_caixa, // a_receber = total_caixa
            $user_id
        ]);
    }
    fclose($handle);

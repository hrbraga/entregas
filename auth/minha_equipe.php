        ...
        if (!empty($novo_username) && !empty($nova_senha)) {
            try {
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $db_users->prepare("INSERT INTO user (username, password_hash, perfil, id_dono, id_loja)VALUES (?, ?, ?, ?, ?)");
                    $stmt->execute([$novo_username, $senha_hash, $perfil, $id_dono, $id_loja]);
